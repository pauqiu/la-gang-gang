...
static std::string authIp, proxyIp, storageIp;
static int authPort = 0, proxyPort = 0, storagePort = 0;
...
void loadEndpoints(const std::string& path) {
    ...
    for (int i = 0; i < 3; ++i) {
        if (!std::getline(file, lines[i]))
            throw std::runtime_error("El archivo debe contener 3 líneas (Auth, Proxy, Storage)");
        if (!lines[i].empty() && lines[i].back() == '\r')
            lines[i].pop_back(); // quitar CR
    }
