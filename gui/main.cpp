...
static void injectSampleUsers(FileSystem &fs) {
    ...
    std::cout << "Users.txt poblado con " << sampleUsers.size() << " usuarios.\n";
}
...
static void injectSampleRoles(FileSystem &fs) {
    ...
    std::cout << "roles.txt poblado con " << sampleRoles.size() << " roles.\n";
}
...
static void injectSamplePermissions(FileSystem &fs) {
    ...
    // Estructura simple de ejemplo: id;permission
...
static void injectSampleData(FileSystem &fs) {
    std::cout << "Primer inicio detectado: inyectando datos de prueba...\n";
    injectSampleUsers(fs);
    injectSampleRoles(fs);
    injectSamplePermissions(fs);
    std::cout << "Inyección completada.\n";
}
...
int main(int argc, char *argv[])
{
    ...
    bool firstRun = !fileExists(diskName);

    FileSystem fs(diskName);
    if (firstRun) {
        injectSampleData(fs);
    } else {
        std::cout << "disk.bin ya existe, no se inyectan datos.\n";
    }
